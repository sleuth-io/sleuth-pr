...
from sleuthpr.models import Installation, Repository
...
def index(request):
    ...
    return HttpResponse(f"Hello, world. You're at the sleuthpr app. - {private_key}")
...
def welcome(request):
    ...
    jwt_token = gen_jwt()
    resp = requests.post(headers={"Authorization": f"Bearer {jwt_token}", "Accept": "application/vnd.sleuthpr.v3+json"},
...
def event(request):
    ...
    if event_name == "installation":
        ...
        if action == "created":
            ...
            installation = Installation.objects.create(remote_id=installation_id, target_type=target_type,
                                                       target_id=target_id)
